...
//   const [request_no, setReqeuestNo] = useState<number>();

//   useEffect(() => {
//     // Fetch data from the server when the component mounts
//     const fetchData = async () => {
//       try {
//         const response = await axios.get("http://localhost:8000/api/request_status/1/");
...
const SmForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/request-details/${request_no}/`);
        const data = response.data;
        ...
        if (Array.isArray(data)) {
          // Assuming data is an array of FormDetails
      ...
      <div className="d-flex justify-content-between align-items-center">
        <div className="w-50">
          ...
          <div className="form-entry">
            ...
            <select
              value={buildingNo ?? ''}
              onChange={(e) => editable && setBuildingNo(e.target.value ? Number(e.target.value) : undefined)}
              className="half-width-input"
              disabled={!editable}
            >
              <option value="">Select Building No.</option>
